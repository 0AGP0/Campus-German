---
title: "Akşam Yarı Yoğun Almanca Kursları"
lang: "tr"
customSlug: "aksam-yari-yogun"
badge:
  icon: "fas fa-moon"
  text: "Akşam Kursu"
description: "Çalışanlar ve öğrenciler için akşam saatlerinde yarı yoğun Almanca kursu. İş sonrası Almanca öğrenin."
image: "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?ixlib=rb-4.0.3&auto=format&fit=crop&w=2071&q=80"
features:
  - icon: "fas fa-clock"
    title: "Haftalık Ders"
    description: "10-12 Ders"
  - icon: "fas fa-calendar-alt"
    title: "Ders Saatleri"
    description: "Akşam"
  - icon: "fas fa-users"
    title: "Sınıf Mevcudu"
    description: "Maksimum 12 Kişi"
  - icon: "fas fa-briefcase"
    title: "Hedef Kitle"
    description: "Çalışanlar"
heroButtons:
  primary:
    text: "Hemen Başvur"
    icon: "fas fa-arrow-right"
    href: "/booking"
  secondary:
    text: "Ders Programı"
    icon: "fas fa-download"
    href: "#ders-programi"
tabs:
  - id: "kurs-bilgileri"
    icon: "fas fa-info-circle"
    title: "Kurs Bilgileri"
    content:
      title: "Akşam Yarı Yoğun Kurs Hakkında"
      paragraphs:
        - heading: "Kursa Genel Bakış"
          text: "Akşam Yarı Yoğun Almanca Kursu, gündüzleri çalışan veya eğitim gören ancak Almanca öğrenmeyi ertelemek istemeyen katılımcılar için özel olarak tasarlanmış esnek bir eğitim programıdır. CampusGerman tarafından sunulan bu kurs, akşam saatlerinde düzenlenen derslerle katılımcıların günlük programlarını bozmadan Almanca öğrenmelerini sağlar. Yarı yoğun format, haftalık düzenli ders temposu sayesinde dil öğrenimini sürdürülebilir hâle getirir. Program, CEFR (Avrupa Ortak Dil Referans Çerçevesi) standartlarına uygun olarak yapılandırılır ve katılımcıların seviyelerine göre A1'den ileri seviyelere kadar uyarlanabilir. Derslerde amaç, kısa sürede aşırı yük bindirmek yerine istikrarlı ve kalıcı bir ilerleme sağlamaktır. CampusGerman Akşam Yarı Yoğun Almanca Kursu, iş–özel hayat–eğitim dengesini koruyarak Almanca öğrenmek isteyenler için ideal bir çözümdür."
        - heading: "Hedef Kitle ve Ön Koşullar"
          text: "Bu kurs, gündüz saatlerinde yoğun programı olan bireyler için uygundur. Özellikle aşağıdaki gruplara hitap eder: Tam zamanlı veya yarı zamanlı çalışanlar Üniversite öğrencileri Akşam saatlerinde Almanca öğrenmek isteyenler Yoğun programlara katılamayan ancak düzenli ilerlemek isteyenler Kursa katılım için belirli bir başlangıç seviyesi zorunlu değildir. Programlar A1'den B2/C1 seviyelerine kadar farklı düzeylerde sunulabilir. CampusGerman, kurs öncesinde seviye değerlendirmesi yaparak katılımcıların uygun sınıflara yerleştirilmesini sağlar."
        - heading: "Kurs İçeriği ve Öğrenme Hedefleri"
          text: "Akşam Yarı Yoğun Almanca Kursu, genel Almanca becerilerini dengeli şekilde geliştirmeyi hedefler. Kurs içeriğinde: Günlük ve iş hayatında kullanılan Almanca Temel ve ileri seviye gramer yapıları Konuşma ve dinleme pratiği Okuma ve yazma çalışmaları Kelime dağarcığını geliştiren alıştırmalar yer alır. Öğrenme hedefi, katılımcının Almanca'yı günlük yaşamda ve akademik/mesleki bağlamda daha rahat kullanabilmesidir. Kurs sonunda öğrenciler, bir üst seviyeye geçiş yapabilecek sağlam bir dil altyapısına sahip olur."
        - heading: "Öğretim Metodu ve Sınıf Ortamı"
          text: "CampusGerman'da Akşam Yarı Yoğun Almanca Kursu, iletişim odaklı ve yetişkin öğrenimine uygun bir öğretim yaklaşımıyla yürütülür. Derslerde: Canlı konuşma ve diyalog çalışmaları Grup ve eşli aktiviteler Günlük hayattan örnekler Düzenli tekrar ve pekiştirme kullanılır. Sınıf ortamı destekleyici ve rahattır. Akşam saatlerine uygun tempo sayesinde katılımcılar derse daha motive şekilde katılabilir. Eğitmenler, yetişkin gruplarla çalışma konusunda deneyimlidir."
        - heading: "Kurs Sonrası İlerleme"
          text: "Akşam Yarı Yoğun Almanca Kursu'nu tamamlayan katılımcılar: Almanca iletişim becerilerinde istikrarlı gelişim sağlar Bir üst seviyedeki Almanca kurslarına geçiş yapabilir İş, eğitim veya günlük yaşamda Almanca'yı daha rahat kullanır Yoğun veya sınav hazırlık programlarına geçiş için hazır hâle gelir CampusGerman, kurs sonrasında da katılımcıları ileri seviye Almanca programları ve kişisel hedeflerine uygun eğitim seçenekleri konusunda bilgilendirir."
  - id: "tarihler-fiyatlar"
    icon: "fas fa-calendar-alt"
    title: "Tarihler ve Fiyatlar"
    content:
      title: "Fiyat Bilgileri"
      pricing:
        - name: "Akşam Yarı Yoğun"
          duration: "Esnek"
          schedule: "10-12 Ders/Hafta"
          price: "Özel Fiyatlandırma"
          highlighted: true
  - id: "dersler"
    icon: "fas fa-book-open"
    title: "Dersler"
    content:
      title: "Ders Programı"
      lessons:
        - title: "18:00 - 19:30"
          description: "Dilbilgisi ve kelime çalışması"
        - title: "19:30 - 19:45"
          description: "Ara"
        - title: "19:45 - 20:30"
          description: "Konuşma pratiği ve etkileşim"
faqs:
  - question: "Hangi günler ders var?"
    answer: "Genellikle Pazartesi, Çarşamba ve Cuma akşamları ders yapılır."
  - question: "Tüm seviyeler için mevcut mu?"
    answer: "Evet, A1'den C1'e kadar tüm seviyeler için akşam kursları mevcuttur."
  - question: "Hangi kurs formatları sunulmaktadır?"
    answer: "Kurslarımız size esnek bir öğrenme deneyimi sunar: Hibrit Eğitim: Kursunuz, yüz yüze dersler ile canlı online derslerin birleşiminden oluşur. Hibrit sistem sayesinde, yüz yüze ders günlerinde dilediğiniz zaman online olarak da derse katılabilirsiniz. Canlı Online Eğitim: Bu formatta derslere tamamen canlı ve online olarak katılırsınız."
  - question: "Kurslarda hangi içerikler işlenmektedir?"
    answer: "Yoğun Almanca programlarımız; güncel metinlerle dinleme ve okuma anlama, kapsamlı gramer çalışmaları ve çeşitli iletişim odaklı aktiviteler içerir. Bu sayede özellikle konuşma ve yazma becerilerinizin hedefe yönelik olarak geliştirilmesi amaçlanır."
  - question: "Almanya'da nasıl üniversite eğitimi alabilirim?"
    answer: "Almanya'da üniversite eğitimi alabilmek için öncelikle tanınmış bir üniversiteye giriş yeterliliğine (HZB) sahip olmanız gerekir. Diplomanızın Almanya'da tanınıp tanınmadığını anabin sistemi üzerinden kontrol etmeniz önerilir. Ayrıca Almanca dil yeterliliğinizi telc, DSH gibi resmi sertifikalarla belgelemeniz gerekir. Campus German Üniversite Hazırlık Programı, bu sürece eksiksiz şekilde hazırlanmanız için tasarlanmıştır."
  - question: "Hangi şartlar gereklidir?"
    answer: "En az 18 yaşında olmanız gerekir. Almanya'da açılmış bir blokeli hesap veya resmî taahhüt belgesi sunmanız gerekmektedir."
  - question: "Hangi vize türleri önerilmektedir?"
    answer: "Öğrenci Vizesi: Hazırlık Almanca kurslarına katılım hakkını da kapsar. AB vatandaşları için vize zorunluluğu bulunmamaktadır."
  - question: "Kurslara hangi hizmetler dahildir?"
    answer: "Seviye tespit sınavı, kayıt ücreti alınmaz, vize başvurusu için kurs belgesi, dijital ders kitabı, bireysel öğrenci danışmanlığı (1 kez, yaklaşık 45 dakika), 'Almanya'da Üniversite Eğitimi' konulu webinarlar, Campus German sertifikası, e-öğrenme platformu, yıl boyunca düzenlenen sosyal ve kültürel aktiviteler, tüm merkezlerde ücretsiz Wi-Fi erişimi."
  - question: "Eğitmenlerimizin nitelikleri nelerdir?"
    answer: "Eğitmen kadromuz; üniversite mezunu, Yabancı Dil Olarak Almanca (DaF) ve İkinci Dil Olarak Almanca (DaZ) alanlarında uzun yıllara dayanan öğretim deneyimine sahip, alanında uzman ve motivasyonu yüksek kişilerden oluşur. Ayrıca Federal Göç ve Mülteciler Dairesi (BAMF) onaylı eğitmenlerimiz bulunmaktadır. Derslerimiz düzenli olarak öğrenciler tarafından değerlendirilir."
  - question: "Almanca öğrenmek ne kadar sürer?"
    answer: "Almanca öğrenme süresi hedefinize ve mevcut seviyenize bağlıdır. A1'den B2'ye kadar olan temel seviyeleri 6-8 ayda tamamlayabilirsiniz. C1 ve C2 gibi ileri seviyeler için ek 4-6 ay gerekebilir. Yoğun kurslarımızla bu süreyi kısaltabilirsiniz."
  - question: "Goethe sertifikası alabilir miyim?"
    answer: "Evet, CampusGerman'da Goethe sertifikası hazırlık kursları sunuyoruz. A1'den C2'ye kadar tüm seviyelerde sınav hazırlığı yapıyoruz. Sertifika sınavlarına kampüsümüzde girebilir veya yakındaki test merkezlerine yönlendirilebilirsiniz."
  - question: "Konaklama seçenekleri mevcut mu?"
    answer: "Evet, Almanya'da konaklama seçenekleri sunuyoruz. Öğrenci yurtları, aile yanı konaklama ve paylaşımlı daireler mevcuttur. Konaklama hizmetimiz kurs ücretine dahil değildir ancak size en uygun seçenekleri bulmanızda yardımcı oluyoruz."
  - question: "Ödeme planları ve indirimler mevcut mu?"
    answer: "Evet, esnek ödeme planları sunuyoruz. Aylık, 3 aylık ve 6 aylık ödeme seçenekleri mevcuttur. Erken kayıt, grup kayıtları ve uzun dönem programlar için indirimler bulunmaktadır. Detaylı bilgi için bizimle iletişime geçin."
hasDemandForm: true
---

<!-- İçerik buraya eklenecek -->

