---
title: "Erasmus+ Öğretmenlere Almanca Kursu"
lang: "tr"
customSlug: "erasmus-ogretmenler"
badge:
  icon: "fas fa-chalkboard-teacher"
  text: "Erasmus+"
description: "Erasmus+ öğretmenleri için özel Almanca kursu. Eğitim sektörü odaklı içerik ve esnek program."
image: "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?ixlib=rb-4.0.3&auto=format&fit=crop&w=2071&q=80"
features:
  - icon: "fas fa-clock"
    title: "Haftalık Ders"
    description: "20 Ders (UE)"
  - icon: "fas fa-calendar-alt"
    title: "Kurs Süresi"
    description: "2-4 Hafta"
  - icon: "fas fa-users"
    title: "Sınıf Mevcudu"
    description: "Maksimum 12 Kişi"
  - icon: "fas fa-chalkboard-teacher"
    title: "Hedef"
    description: "Öğretmenler"
heroButtons:
  primary:
    text: "Hemen Başvur"
    icon: "fas fa-arrow-right"
    href: "/tr/buchung"
  secondary:
    text: "Erasmus+ Bilgileri"
    icon: "fas fa-info-circle"
    href: "#erasmus-bilgisi"
tabs:
  - id: "kurs-bilgileri"
    icon: "fas fa-info-circle"
    title: "Kurs Bilgileri"
    content:
      title: "Erasmus+ Öğretmenler Kursu Hakkında"
      paragraphs:
        - heading: "Kursa Genel Bakış"
          text: "Erasmus Öğretmenleri İçin Almanca Kursu, Erasmus öğretim elemanı hareketliliği kapsamında Almanya'ya gidecek akademisyenler ve öğretmenler için özel olarak hazırlanmış profesyonel bir dil programıdır. CampusGerman tarafından sunulan bu kurs, katılımcıların Almanya'daki akademik ortama hızlı şekilde uyum sağlamasını ve öğretim faaliyetlerini Almanca destekli olarak yürütebilmesini hedefler. Erasmus öğretmenleri için Almanca bilgisi, ders anlatımı, akademik sunumlar, üniversite içi iletişim ve meslektaşlarla etkileşim açısından büyük önem taşır. Bu kurs, genel Almanca bilgisini akademik dil, üniversite terminolojisi ve öğretim odaklı iletişimle birleştirerek katılımcılara hedefe yönelik bir hazırlık sunar. Program, CEFR standartlarıyla uyumlu şekilde yapılandırılır ve öğretmenlerin mevcut seviyelerine göre esnek biçimde uygulanır. CampusGerman Erasmus Öğretmenleri İçin Almanca Kursu, kısa sürede maksimum fayda sağlayan akademik ve mesleki odaklı bir eğitim programıdır."
        - heading: "Hedef Kitle ve Ön Koşullar"
          text: "Bu kurs, Erasmus kapsamında Almanya'da ders verme veya akademik faaliyetlerde bulunacak öğretim elemanları için uygundur. Özellikle aşağıdaki gruplara hitap eder: Üniversitelerde görev yapan akademisyenler Erasmus öğretim elemanı hareketliliğine katılacak öğretmenler Almanya'daki üniversitelerde ders veya seminer verecek adaylar Akademik iletişimde Almanca kullanması gereken eğitimciler Kursa katılım için en az B1 seviyesinde Almanca bilgisi önerilir. Bu seviye, akademik iletişim ve ders anlatımı gibi konularda daha verimli bir öğrenme süreci sağlar. CampusGerman, kurs öncesinde seviye ve ihtiyaç analizi yaparak katılımcıları en uygun programa yönlendirir."
        - heading: "Kurs İçeriği ve Öğrenme Hedefleri"
          text: "Erasmus Öğretmenleri İçin Almanca Kursu, akademik öğretim ve üniversite ortamına yönelik dil becerilerine odaklanır. Kurs içeriğinde: Yoğun Almanca ve üniversite terminolojisi Ders anlatımı ve sunum dili Akademik toplantı ve seminer iletişimi Resmî yazışmalar ve e-posta dili Akademik tartışma ve soru-cevap teknikleri yer alır. Öğrenme hedefi, katılımcının Almanca'yı akademik ortamda net, doğru ve profesyonel şekilde kullanabilmesidir."
        - heading: "Öğretim Metodu ve Sınıf Ortamı"
          text: "CampusGerman'da bu kurs, akademik uygulama ve iletişim odaklı bir öğretim yaklaşımıyla yürütülür. Derslerde: Akademik sunum çalışmaları Rol canlandırmaları ve ders anlatım simülasyonları Gerçek üniversite senaryoları Bireysel geri bildirimler kullanılır. Sınıf ortamı etkileşimlidir ve katılımcıların aktif rol alması teşvik edilir. Eğitmenler, yoğun Almanca ve yükseköğretim sistemi konusunda deneyimlidir."
        - heading: "Kurs Sonrası İlerleme"
          text: "Erasmus Öğretmenleri İçin Almanca Kursu'nu tamamlayan katılımcılar: Almanya'daki akademik ortama daha hızlı uyum sağlar Ders ve seminerlerde Almanca'yı daha güvenle kullanır Akademik iletişim becerilerini geliştirir İleri seviye yoğun Almanca programlarına geçiş yapabilir CampusGerman, kurs sonrası öğretmenleri ileri seviye akademik ve mesleki Almanca programları konusunda da bilgilendirmeye devam eder."
  - id: "tarihler-fiyatlar"
    icon: "fas fa-calendar-alt"
    title: "Tarihler ve Fiyatlar"
    content:
      title: "Fiyat Bilgileri"
      pricing:
        - name: "2 Hafta Erasmus+"
          duration: "2 Hafta"
          schedule: "20 Ders/Hafta"
          price: "Özel Fiyatlandırma"
        - name: "4 Hafta Erasmus+"
          duration: "4 Hafta"
          schedule: "20 Ders/Hafta"
          price: "Özel Fiyatlandırma"
          highlighted: true
  - id: "erasmus-bilgisi"
    icon: "fas fa-chalkboard-teacher"
    title: "Erasmus+ Bilgileri"
    content:
      title: "Erasmus+ Öğretmenler İçin Özellikler"
      items:
        - icon: "fas fa-check-circle"
          text: "<strong>Eğitim Odaklı:</strong> Pedagojik terimler ve eğitim Almancası"
        - icon: "fas fa-check-circle"
          text: "<strong>Erasmus+ Uyumlu:</strong> Erasmus+ program gereksinimlerine uygun"
        - icon: "fas fa-check-circle"
          text: "<strong>Sertifika:</strong> Erasmus+ sertifikası verilir"
faqs:
  - question: "Erasmus+ öğretmeni olduğumu nasıl kanıtlarım?"
    answer: "Erasmus+ belgenizi kayıt sırasında sunmanız yeterlidir."
  - question: "Eğitim sektörü odaklı içerik var mı?"
    answer: "Evet, kurs pedagojik terimler ve eğitim sektörü odaklı Almanca içerir."
  - question: "Hangi kurs formatları sunulmaktadır?"
    answer: "Kurslarımız size esnek bir öğrenme deneyimi sunar: Hibrit Eğitim: Kursunuz, yüz yüze dersler ile canlı online derslerin birleşiminden oluşur. Hibrit sistem sayesinde, yüz yüze ders günlerinde dilediğiniz zaman online olarak da derse katılabilirsiniz. Canlı Online Eğitim: Bu formatta derslere tamamen canlı ve online olarak katılırsınız."
  - question: "Kurslarda hangi içerikler işlenmektedir?"
    answer: "Yoğun Almanca programlarımız; güncel metinlerle dinleme ve okuma anlama, kapsamlı gramer çalışmaları ve çeşitli iletişim odaklı aktiviteler içerir. Bu sayede özellikle konuşma ve yazma becerilerinizin hedefe yönelik olarak geliştirilmesi amaçlanır."
  - question: "Almanya'da nasıl üniversite eğitimi alabilirim?"
    answer: "Almanya'da üniversite eğitimi alabilmek için öncelikle tanınmış bir üniversiteye giriş yeterliliğine (HZB) sahip olmanız gerekir. Diplomanızın Almanya'da tanınıp tanınmadığını anabin sistemi üzerinden kontrol etmeniz önerilir. Ayrıca Almanca dil yeterliliğinizi telc, DSH gibi resmi sertifikalarla belgelemeniz gerekir. Campus German Üniversite Hazırlık Programı, bu sürece eksiksiz şekilde hazırlanmanız için tasarlanmıştır."
  - question: "Hangi şartlar gereklidir?"
    answer: "En az 18 yaşında olmanız gerekir. Almanya'da açılmış bir blokeli hesap veya resmî taahhüt belgesi sunmanız gerekmektedir."
  - question: "Hangi vize türleri önerilmektedir?"
    answer: "Öğrenci Vizesi: Hazırlık Almanca kurslarına katılım hakkını da kapsar. AB vatandaşları için vize zorunluluğu bulunmamaktadır."
  - question: "Kurslara hangi hizmetler dahildir?"
    answer: "Seviye tespit sınavı, kayıt ücreti alınmaz, vize başvurusu için kurs belgesi, dijital ders kitabı, bireysel öğrenci danışmanlığı (1 kez, yaklaşık 45 dakika), 'Almanya'da Üniversite Eğitimi' konulu webinarlar, Campus German sertifikası, e-öğrenme platformu, yıl boyunca düzenlenen sosyal ve kültürel aktiviteler, tüm merkezlerde ücretsiz Wi-Fi erişimi."
  - question: "Eğitmenlerimizin nitelikleri nelerdir?"
    answer: "Eğitmen kadromuz; üniversite mezunu, Yabancı Dil Olarak Almanca (DaF) ve İkinci Dil Olarak Almanca (DaZ) alanlarında uzun yıllara dayanan öğretim deneyimine sahip, alanında uzman ve motivasyonu yüksek kişilerden oluşur. Ayrıca Federal Göç ve Mülteciler Dairesi (BAMF) onaylı eğitmenlerimiz bulunmaktadır. Derslerimiz düzenli olarak öğrenciler tarafından değerlendirilir."
  - question: "Almanca öğrenmek ne kadar sürer?"
    answer: "Almanca öğrenme süresi hedefinize ve mevcut seviyenize bağlıdır. A1'den B2'ye kadar olan temel seviyeleri 6-8 ayda tamamlayabilirsiniz. C1 ve C2 gibi ileri seviyeler için ek 4-6 ay gerekebilir. Yoğun kurslarımızla bu süreyi kısaltabilirsiniz."
  - question: "Goethe sertifikası alabilir miyim?"
    answer: "Evet, CampusGerman'da Goethe sertifikası hazırlık kursları sunuyoruz. A1'den C2'ye kadar tüm seviyelerde sınav hazırlığı yapıyoruz. Sertifika sınavlarına kampüsümüzde girebilir veya yakındaki test merkezlerine yönlendirilebilirsiniz."
  - question: "Konaklama seçenekleri mevcut mu?"
    answer: "Evet, Almanya'da konaklama seçenekleri sunuyoruz. Öğrenci yurtları, aile yanı konaklama ve paylaşımlı daireler mevcuttur. Konaklama hizmetimiz kurs ücretine dahil değildir ancak size en uygun seçenekleri bulmanızda yardımcı oluyoruz."
  - question: "Ödeme planları ve indirimler mevcut mu?"
    answer: "Evet, esnek ödeme planları sunuyoruz. Aylık, 3 aylık ve 6 aylık ödeme seçenekleri mevcuttur. Erken kayıt, grup kayıtları ve uzun dönem programlar için indirimler bulunmaktadır. Detaylı bilgi için bizimle iletişime geçin."
---

<!-- İçerik buraya eklenecek -->

